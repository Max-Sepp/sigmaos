...
func TestImgrec(t *testing.T) {
	...
	conf := imgrec_wasm.NewImgrecWASMJobConfig(IMG_BUCKET, IMG_KEY, MODEL_BUCKET, MODEL_KEY, KID, false, false, 0)
...
func TestImgrecWASMCoSandboxShmem(t *testing.T) {
	...
	conf := imgrec_wasm.NewImgrecWASMJobConfig(IMG_BUCKET, IMG_KEY, MODEL_BUCKET, MODEL_KEY, KID, true, true, proc.Tmem(32))
	job, err := imgrec_wasm.NewImgrecWASMJob(conf, rts.SigmaClnt)
	if !assert.Nil(t, err, "NewImgrecWASMJob: %v", err) {
		return
	}
...
func TestImgrecWASMCoSandbox(t *testing.T) {
	...
	conf := imgrec_wasm.NewImgrecWASMJobConfig(IMG_BUCKET, IMG_KEY, MODEL_BUCKET, MODEL_KEY, KID, true, true, 0)
	job, err := imgrec_wasm.NewImgrecWASMJob(conf, rts.SigmaClnt)
	if !assert.Nil(t, err, "NewImgrecWASMJob: %v", err) {
		return
	}
