...
//
//	p := proc.NewProc("memcachedg-shim", []string{
//		resolvedPn,
//		"memcachedg-proc",
...
//		fmt.Sprintf("http://127.0.0.1:%v", CLNT_PORT),
//	})
//	// Add the memcachedg binary to be downloaded with the proc
//	p.AddBin("memcachedg-v1.0")
//	splitFN := strings.Split(resolvedPn, "/")
//	// Read the boot script
//	bootScript, err := memcachedg.GetBootScript(mrts.GetRealm(test.REALM1).SigmaClnt)
//	if !assert.Nil(t, err, "Err read bootscript: %v", err) {
//		return
//	}
//	// Construct the input to the bootscript
//	bootScriptInput, err := memcachedg.GetBootScriptInput(splitFN[0], filepath.Join(splitFN[1:]...), sp.LOCAL)
