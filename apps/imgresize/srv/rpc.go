...
func NewRPCSrv(imgd *ImgSrv) *ImgSrvRPC {
	...
	rpcs.mkProc = imgresize.GetMkProcFn(imgd.ftclnt.ServiceId(), imgd.nrounds, imgd.imgDim, imgd.workerMcpu, imgd.workerMem, imgd.workerBootScriptMcpu, imgd.workerBootScriptMem, imgd.bootScript, imgd.bootScriptWriteOut, imgd.useSPProxy, imgd.premountS3, imgd.s3EP)
	return rpcs
}
...
func (rpcs *ImgSrvRPC) Resize(ctx fs.CtxI, req proto.ImgResizeReq, rep *proto.ImgResizeRep) error {
	...
	t := imgresize.NewTask(req.InputPath, req.UseS3Clnt, rpcs.imgd.useBootScript, rpcs.imgd.writeOutViaBootScript)
