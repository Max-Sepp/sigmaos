...
static sigmaos::proxy::s3::Clnt* s3_clnt(SigmaosClntState* st) {
    if (!st->s3) {
        ...
        st->s3 = std::make_shared<sigmaos::proxy::s3::Clnt>(
            st->sp, S3_SVC_BASE + kid);
    }
    return st->s3.get();
}
...
static sigmaos::proxy::ux::Clnt* ux_clnt(SigmaosClntState* st) {
    if (!st->ux) {
        std::string kid = st->sp->ProcEnv()->GetKernelID();
        st->ux = std::make_shared<sigmaos::proxy::ux::Clnt>(
            st->sp, UX_SVC_BASE + kid);
    }
    return st->ux.get();
}
...
char* sigmaos_s3_get_object(SigmaosClnt clnt, const char* bucket,
    ...
    clear_error();
    auto res = s3_clnt(state(clnt))->GetObject(
        std::string(bucket), std::string(key), cache != 0);
    if (!res.has_value()) {
        set_error(res.error().String());
        *out_len = 0;
        return nullptr;
...
int sigmaos_s3_put_object(SigmaosClnt clnt, const char* bucket,
    ...
    clear_error();
    std::string data_str(data, len);
    auto res = s3_clnt(state(clnt))->PutObject(
        std::string(bucket), std::string(key), &data_str);
    if (!res.has_value()) {
        set_error(res.error().String());
        return -1;
    }
