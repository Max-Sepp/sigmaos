...
type HotelBenchConfig struct {
	...
	EvictBenchCfg   *EvictionBenchConfig  `json:"evict_bench_cfg"`
}

func (cfg *HotelBenchConfig) String() string {
	return fmt.Sprintf("&{ JobCfg:%v Durs:%v MaxRPS:%v ScaleGeo:%v MatchUseCaching:%v CachedUserFrac:%v CacheBenchCfg:%v CosSimBenchCfg:%v EvictBenchCfg:%v }",
		cfg.JobCfg, cfg.Durs, cfg.MaxRPS, cfg.ScaleGeo, cfg.MatchUseCaching, cfg.CachedUserFrac, cfg.CacheBenchCfg, cfg.CosSimBenchCfg, cfg.EvictBenchCfg)
}
